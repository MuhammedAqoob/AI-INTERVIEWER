...
async function getDashboardSummary(userId) {
  ...
  return { interviewsRemainingToday: Math.max(0, 5 - (usage?.interviewsStarted || 0)), totalSessions: sessions.length, maxSessions: 15, completedInterviews: sessions.filter((s) => s.status === 'COMPLETED').length, leaderboardEligibleSessions, interviewCounts: counts, continueSessionId: paused?.id || null, analytics: groupedAnalytics(score), averageScore: score.averageScore, categoryScores: score.categoryScores };
}

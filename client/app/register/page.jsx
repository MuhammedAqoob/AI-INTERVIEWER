...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await auth.register(formData.username, formData.password);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950 p-4 transition-colors">
      {/* Keep the auth page from feeling like a dead-end: a direct way back home. */}
      <Link
        href="/"
        className="absolute top-4 left-4 inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 transition-colors rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
      >
